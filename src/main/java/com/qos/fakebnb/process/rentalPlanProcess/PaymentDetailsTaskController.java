...
@RestController
@RequestMapping("/api/rental-plan-process/payment-details-task")
public class PaymentDetailsTaskController {
    ...
    public ResponseEntity<Void> complete(@RequestBody PaymentDetailsTaskContextDTO paymentDetailsTaskContext) {
        log.debug("REST request to complete RentalPlanProcess.PaymentDetailsTask {}", paymentDetailsTaskContext.getTaskInstance().getId());
