...
export default class NotifyMixin extends Vue {
  ...
  public getErrorMessageFromResponse(error: any): any {
    const paramLength: number = error.response.headers['x-fakebnbapp-param-length'];
    if (!paramLength || paramLength == 0) {
      return this.$t(error.response.headers['x-fakebnbapp-error']);
    }

    if (paramLength == 1) {
      return this.$t(error.response.headers['x-fakebnbapp-error'], {
        param: decodeURIComponent(error.response.headers['x-fakebnbapp-param'].replace(/\+/g, ' ')),
      });
    }

    const params = {};
    for (let i = 0; i < paramLength; i++) {
      params['param' + i] = decodeURIComponent(error.response.headers['x-fakebnbapp-param' + i].replace(/\+/g, ' '));
    }
    return this.$t(error.response.headers['x-fakebnbapp-error'], params);
  }

  public getAlertMessageFromResponse(response: any): any {
    return this.$t(response.headers['x-fakebnbapp-alert'], {
      param: decodeURIComponent(response.headers['x-fakebnbapp-params'].replace(/\+/g, ' ')),
    });
  }
}
